...
void construct_adj_list(int** adj_mat, int rows, int cols, adj_node_t*** list)
{
    ...
    for (int i = 0; i < rows; i++) {
	    for (int j = 0; j < cols; j++) {
		    if (adj_mat[i][j] != 0) {     // edge (i, j) exists
         ...
         int* color, int* distance, int* parent)
{
    ...
    add_node(&queue, 0, src_node);      // row index 0 for single queue
    ...
    while (queue != NULL) {
	    ...
	    adj_node_t *neighbor = list[u];
	    while (neighbor != NULL) {
		    int v = neighbor->vid;
		    if (color[v] == 0) {
			    color[v] = 1;   // change white --> gray
		 	    distance[v] = distance[u] + 1;
			    parent[v] = u;
			    adj_node_t *new_node = create_node(v);
