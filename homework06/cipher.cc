...
Cipher::Cipher(string cipher_alpha)
{
	...
	// Validate alpha, then initialize
...
string Cipher::encrypt(string raw)
{
    ...
    for (char c : raw) {
	    if (c == ' ') {		// Preserve space
		    ...
		    // Check case
		    if (isupper(c)) {
			    // Find cipher character
			    i = c - 'A';
			    retStr += UPPER_CASE(smile->cipher_alpha[i]);
		    } else {
			    i = c - 'a';
			    retStr += LOWER_CASE(smile->cipher_alpha[i]);
		    }
	    }
    }
...
string Cipher::decrypt(string enc)
{
    ...
    for (char c : enc) {
	    if (c == ' ') {
		    ...
		    char lower_c = LOWER_CASE(c);
		    unsigned int i = find_pos(smile->cipher_alpha, lower_c);
		    if (isupper(c)) {
			    retStr += UPPER_CASE('a' + i);
		    } else {
			    retStr += LOWER_CASE('a' + i);
		    }
	    }
    }
...
string Cipher::get_cipher_alpha() const {
	return smile->cipher_alpha;
}

void Cipher::set_cipher_alpha(string& alpha) {
	smile->cipher_alpha = alpha;
}
