...
bool Response::checkAndRespond(const string& inWord, ostream& toWhere)
{
	...
	string norm = keyword.normal();   // Initialize keyword as string obj
	string upper = keyword.upper();
	string lower = keyword.lower();   // Modify keyword string instance
	
	// Use find function on all variations of keyword to check if keyword is in inWord
	if (inWord.find(norm) != string::npos ||             // string::npos = max possible position for a char in a given string
		inWord.find(upper) != string::npos ||
		inWord.find(lower) != string::npos) {
		
		// Send response to toWhere
		respond(toWhere);
