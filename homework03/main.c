...

{
	...
	*csr_vals = (double *) malloc(nnz * sizeof(double));
	...
	if (!*csr_row_ptr || !*csr_col_ind || !*csr_vals) {
		if (*csr_row_ptr) free(*csr_row_ptr);
		if (*csr_col_ind) free(*csr_col_ind);
		if (*csr_vals) free(*csr_vals);
		exit(EXIT_FAILURE);
	}

	// 1. Build histogram for pointer offsets
	for (int i = 0; i < nnz; ++i) {
		int row = row_ind[i];
		(*csr_row_ptr)[row] += 1;
	}

	// 2. Calclate prefix sum for pointer values
	unsigned int sum = 0;

	for (int i = 0; i < m; ++i) {			
		unsigned int temp = (*csr_row_ptr)[i];
		(*csr_row_ptr)[i] = sum;
		sum += temp;
	}
	(*csr_row_ptr)[m] = sum;	// Final element

	// 3. Make copy of row ptr to track insertion positions
	unsigned int* row_cpy = (unsigned int *) malloc((m+1) * sizeof(unsigned int));
	...
	memcpy(row_cpy, *csr_row_ptr, (m+1) * sizeof(unsigned int));
	
	// 4. Copy col indices and vals 
	int dest;

	for (int i = 0; i < nnz; ++i) {
		unsigned int row = row_ind[i];			// Get row index for ith element
		dest = row_cpy[row];			// Get current insertion index for this row

		(*csr_col_ind)[dest] = col_ind[i];	// Copy col index to correct index
		(*csr_vals)[dest] = val[i];		// Copy val
		
		row_cpy[row]++; 			// Increment row insertion ptr to next slot
	}
          ...
          double* vector_x, double* res)
{
	for (int i = 0; i < m; ++i) {
		...
		for (unsigned int j = csr_row_ptr[i]; j < csr_row_ptr[i+1]; ++j) {	// Calculate indice range of row i
			sum += csr_vals[j] * vector_x[csr_col_ind[j]];			// Compute & accumulate val * vector multiplication
		}
		res[i] = sum;
	}
